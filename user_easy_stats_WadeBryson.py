...
# Team Points Scored in a Season
season_points_scored = [
   40,
   42,
   50,
   38,
...
logger.info("season_points_scored = " + str(season_points_scored))

# Calculating Averages and measures of central tendency

mean = statistics.mean(season_points_scored)
median = statistics.median(season_points_scored)
mode = statistics.mode(season_points_scored)
...
var = statistics.variance(season_points_scored)
stdev = statistics.stdev(season_points_scored)
lowest = min(season_points_scored)
highest = max(season_points_scored)
